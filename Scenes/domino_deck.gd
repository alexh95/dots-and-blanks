...
const init_domino_values: Array[Vector2i] = [Vector2i(1, 1), Vector2i(1, 2), Vector2i(1, 3), Vector2i(2, 2), Vector2i(2, 3), Vector2i(3, 3)]

var selected_domino: Domino = null

func _ready():
	for valueIndex in range(init_domino_values.size()):
		add_domino(init_domino_values[valueIndex])

func get_domino_at_position(screen_position: Vector2i):
	for domino in $Dominoes.get_children(true):
		if domino.contains_point(screen_position):
			return domino
	return null

func highlight_domino(target_domino: Domino):
	for domino in $Dominoes.get_children(true):
...
func select_domino(target_domino: Domino):
...
func add_domino(dot_values: Vector2i):
	var domino = domino_generator.instantiate()
	domino.putDots(dot_values.x, dot_values.y)
	$Dominoes.add_child(domino)
	recalculate_positions()

func remove_selected_domino():
	if selected_domino != null:
